...
createWidget("discourse-poll-buttons", {
  ...
  html(attrs) {
    ...
    if (attrs.showResults || hideResultsDisabled) {
      ...
      if (poll.get("results") === "vote" && !attrs.hasVoted) {
        contents.push(infoTextHtml(I18n.t("poll.results.vote.title")));
      } else if (poll.get("results") === "closed" && !closed) {
...
export default createWidget("discourse-poll", {
  ...
  defaultState(attrs) {
    ...
    const showResults = (
      ...
      this.isClosed() ||
      (poll.get("results") !== "closed" && this.hasVoted())
    );
  ...
  castVotes() {
    ...
    return ajax("/polls/vote", {
      ...
      if (attrs.poll.get("results") !== "closed") {
